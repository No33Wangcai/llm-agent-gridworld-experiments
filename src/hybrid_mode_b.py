...
def main() -> None:
    parser = argparse.ArgumentParser(description="Text World Demo")
    parser.add_argument("--ai", action="store_true", help="使用模型解析玩家输入")
    parser.add_argument("--solve", action="store_true", help="让模型自动寻找并打开箱子")
    parser.add_argument(
        "--hybrid-solve",
        action="store_true",
        help="模式B：本地小模型每步执行，云端大模型只在卡住时给策略",
    )
    parser.add_argument(
        "--provider",
    ...
    config = build_model_config(args.provider, args.model) if use_ai or solve else None
    ...
    if hybrid_solve and local_config and cloud_config:
        mode = f"hybrid local={local_config.provider}:{local_config.model} cloud={cloud_config.provider}:{cloud_config.model}"
    else:
